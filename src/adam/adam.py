...
class SimpleNetwork:
    ...
    def minibatch_update(self, batch: List[tuple[np.ndarray, np.ndarray]], lr: float):
        ...
        - BP1: δ^L =  ∇_a C ⊙ σ'(z^L)                         Get Error δ in last layer of network ∇_a C for quadratic cost 0.5(y(x) - a^L(x))² -> (a^L - y)
...
class Adam(alpha: float, beta1: float, beta2: float, epsilon: float):
